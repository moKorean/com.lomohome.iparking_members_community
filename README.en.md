...
Store and device images are generated from the sources in `docs/` by
`scripts/make_images.py` — each `.svg` there is a frame, and the artwork it frames is the
photograph or illustration beside it. **Re-run it after editing any of them** — a test fails if you
